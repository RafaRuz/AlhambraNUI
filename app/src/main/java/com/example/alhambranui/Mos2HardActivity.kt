...
class Mos2HardActivity : AppCompatActivity(), SensorEventListener {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_mos2hard)

        sensorManager = getSystemService(Context.SENSOR_SERVICE) as SensorManager
        sensor = sensorManager!!.getDefaultSensor(Sensor.TYPE_LIGHT)

        if(!completados_hard[1]) {
            handler = Handler()

            handler!!.postDelayed(Runnable {
                var layout =
                    findViewById<androidx.constraintlayout.widget.ConstraintLayout>(R.id.mos2_layout)
        ...
        var lib = GestureLibraries.fromRawResource(this, R.raw.gesture_mos2)
        ...
        gesture.addOnGesturePerformedListener { overlay, gesture ->
            ...
            for (prediction in predictionArrayList) {
                if (prediction.score > 3) {
                    txtResult.setVisibility(View.VISIBLE)
                    findViewById<Button>(R.id.boton_atras).setVisibility(View.VISIBLE)
                    var layout = findViewById<androidx.constraintlayout.widget.ConstraintLayout>(R.id.mos2_layout)
                    layout.setBackgroundResource(R.drawable.mos2)
                    completados_hard[1]=true
                    encontrado=true
                    break
                }
            }
    ...
    fun atras(view: View) {
        ...
        intent.putExtra("completed_levels", completados);
